...
- Verify the agent's returned summary contains BOTH `Pending markers remaining: 0` AND `Broken cross-links: 0`. If `Pending markers remaining` is non-zero, the report was truncated mid-generation and is showing only a TOC with empty sections — re-delegate with the explicit instruction: "the previous run left `<!-- pending -->` markers in the file; complete the population phase by editing each remaining placeholder, working in smaller batches if needed." If broken cross-links exist, re-delegate once asking it to fix the dangling anchors (typical causes: an Endpoint field references an EP-ID that recon didn't enumerate, or a SEC-ID was misspelled in a finding's cross-reference).
